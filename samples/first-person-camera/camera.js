                                    ...
                                    })
{
    ...
    return {
        get position()
        {
            return Luu.translation(position.x, position.y, position.z);
        },

        get direction()
        {
            return Luu.rotation(direction.x, direction.y, direction.z);
        },
        ...
        update: function()
        {
            ...
            const forwardVector = Luu.rotation(direction.x, direction.y, direction.z);
    ...
    {
        direction.x += (event.movementY / 300);
        direction.y += (event.movementX / 300);
    ...
    {
        const cameraRotationMatrix = Luu.matrix44.rotation(0, forwardVector.y, 0);

        const accumulatedMovement = Luu.vector3((movementStatus.left?    1 : movementStatus.right?    -1 : 0),
                                                (movementStatus.up?      1 : movementStatus.down?     -1 : 0),
                                                (movementStatus.forward? 1 : movementStatus.backward? -1 : 0));

        Luu.vector3.normalize(accumulatedMovement);
        Luu.vector3.transform(accumulatedMovement, cameraRotationMatrix);
